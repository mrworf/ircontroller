...
parser.add_argument('--pronto', action="store_true", default=False, help="Learning should be done using hex codes")
  ...
  for p in parts:
    ...
    if plearn is None:
      if p == "0000":
        ...
        return None
    elif pcarrier is None:
      punit = int(round(int(p, 16) * 0.241246))
      pcarrier = int(round(1000000/punit))
    elif plead is None:
      plead = int(p, 16)*2
    elif pmain is None:
      pmain = int(p, 16)*2
...
if config.learn and config.file:
  while True:
    ...
    if config.pronto:
      sys.stdout.write("Pronto IR HEX Code (or enter to end): ")
      sys.stdout.flush()
